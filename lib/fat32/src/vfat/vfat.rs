    ...
    pub fn read_cluster(&mut self, cluster: Cluster, offset: usize, buf: &mut [u8]) -> io::Result<usize> {
        ...
        match cluster_entry.status() {
            Status::Data(_) => {
                ...
                let num_bytes_read = (self.bytes_per_sector * self.sectors_per_cluster as u16) as usize - offset;
                buf[..num_bytes_read].clone_from_slice(&cluster_data[offset..]);
        ...
        loop {
            ...
            match curr_entry.status() {
                Status::Data(next) => {
                    ...
                    self.read_cluster(curr_cluster, 0, &mut buf[buf_offset..buf_offset + bytes_per_cluster])?;
                    curr_cluster = next;
                },
                Status::Eoc(marker) => {
                    // grow the buf to accomodate new cluster
            ...
            match curr_entry.status() {
                Status::Data(next) => {
                    if load_buf {
                        ...
                        buf.resize(buf.len() + bytes_per_cluster, 0);
                        // read the cluster into buf
                        self.read_cluster(curr_cluster, cluster_offset, &mut buf[buf_offset..buf_offset + bytes_per_cluster])?;
                    }
                    curr_cluster = next;
                },
                Status::Eoc(marker) => {
